export { }

type Successful = {
    code: 200 | 201 | 301;
    text: string
};

type Unsuccessful = {
    code: 400 | 404 | 500;
    text: string;
    printChars?: number;
};

function httpCodes(httpRequest: Successful | Unsuccessful): void {
    if ('printChars' in httpRequest) {
        console.log(httpRequest.text.substring(0, httpRequest.printChars));
    } else {
        console.log(httpRequest.text);
    }
}

httpCodes({ code: 200, text: 'OK' });
httpCodes({ code: 201, text: 'Created' });
httpCodes({ code: 400, text: 'Bad Request', printChars: 4 });
httpCodes({ code: 404, text: 'Not Found' });
httpCodes({ code: 404, text: 'Not Found', printChars: 3 });
httpCodes({ code: 500, text: 'Internal Server Error', printChars: 1 });